...
// Convert any Figma share URL into a proper embed URL.
// Figma blocks www.figma.com inside iframes — only embed.figma.com (or the /embed endpoint) works.
function toFigmaEmbedUrl(url) {
  ...
  // Only transform Figma domains
  if (!url.includes('figma.com')) return url

  try {
    const u = new URL(url)
    const path = u.pathname // e.g. /board/FILE_KEY/Name or /design/FILE_KEY/Name or /proto/...

    // Match /design/, /board/, /proto/, /file/, /slides/, /deck/
    const match = path.match(/^\/(design|board|proto|file|slides|deck)\/([a-zA-Z0-9]+)/)
    if (match) {
      const type = match[1] === 'file' ? 'design' : match[1] // legacy /file/ → design
      const fileKey = match[2]

      // Preserve useful query params (node-id, starting-point-node-id, etc.)
      const params = new URLSearchParams()
      params.set('embed-host', 'uxapex')
      if (u.searchParams.get('node-id')) {
        params.set('node-id', u.searchParams.get('node-id'))
      }
      if (u.searchParams.get('starting-point-node-id')) {
        params.set('starting-point-node-id', u.searchParams.get('starting-point-node-id'))
      }
      if (u.searchParams.get('page-id')) {
        params.set('page-id', u.searchParams.get('page-id'))
      }

      return `https://embed.figma.com/${type}/${fileKey}?${params.toString()}`
    }

    // Fallback: classic embed endpoint
...
export function DesignCard({ design, index = 0 }) {
  const [modal, setModal] = useState(null) // { url, label } | null

  const dotColor = design.color === '#1A1A1A' || design.color === '#080808'
    ? BLUE : design.color
          ...
          <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', flex: 1 }}>
            ...
            <div style={{
              ...
              <button
                ...
                onMouseEnter={e => e.currentTarget.style.background = '#1448B3'}
              ...
              {showSecondary && (
                <button
                  ...
                  style={{
                    ...
                    minHeight: 42,
                    fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.12em', fontWeight: 600,
                    color: INK_SEC, background: 'transparent',
                    border: '1.5px solid ' + BORDER, borderRadius: 9999,
                  ...
                  onMouseLeave={e => {
                    e.currentTarget.style.color = INK_SEC
                    e.currentTarget.style.borderColor = BORDER
                  }}
                >
                  {secondaryLabel} →
