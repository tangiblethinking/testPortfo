  ...
  return (
    <div role="dialog" aria-modal="true" aria-label={label}
      ...
      <motion.div initial={{ opacity: 0, y: 20, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.28, ease: expo }}
        style={{ width: '100%', maxWidth: 1200, height: '92vh', background: SURF0, borderRadius: 16, overflow: 'hidden', border: '1px solid ' + BORDER, display: 'flex', flexDirection: 'column', boxShadow: '0 24px 80px rgba(0,0,0,0.14)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 1rem', background: SURF1, borderBottom: '1px solid ' + BORDER, flexShrink: 0, height: 48, minHeight: 48 }}>
          ...
          <button onClick={onClose} aria-label="Close" style={{ width: 32, height: 32, borderRadius: '50%', background: SURF2, border: '1px solid ' + BORDER, color: INK_BODY, fontSize: '0.9rem', cursor: 'pointer', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.2s' }}
            onMouseEnter={e => e.currentTarget.style.background = BORDER}
            onMouseLeave={e => e.currentTarget.style.background = SURF2}>✕</button>
        </div>
        <iframe src={url} title={label} style={{ width: '100%', flex: 1, border: 'none', display: 'block' }} allow="fullscreen" />
      </motion.div>
    </div>
  )
}
      ...
      <section aria-label="About" style={{ background: SURF0, padding: SP }}>
        <div style={{ ...MAX, display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 'clamp(2rem,5vw,5rem)', alignItems: 'start' }}>
          ...
          <div>
            ...
            <FadeUp delay={0.1}>
              <p style={{ fontSize: '0.95rem', color: INK_BODY, lineHeight: 1.75, marginBottom: '1rem' }}>Owning major strategic product initiatives, multi-platform user experience visions, and mentoring design teams.</p>
            </FadeUp>
            <FadeUp delay={0.26}>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', alignItems: 'center' }}>
                <PillButton onClick={() => setDesignOpsOpen(true)} />
                <button onClick={() => go(setPage, 'about')} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, minHeight: 44, fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.14em', fontWeight: 600, color: INK, border: '1.5px solid ' + BORDER, borderRadius: 9999, padding: '0.75rem 1.5rem', background: 'transparent', cursor: 'pointer', transition: 'all 0.25s' }}
                  ...
                  onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = INK; e.currentTarget.style.borderColor = BORDER }}>Full Profile →</button>
              </div>
            </FadeUp>
          </div>
        </div>
      ...
      <section aria-label="Selected work" style={{ background: SURF1, padding: SP, borderTop: '1px solid ' + BORDER }}>
        <div style={MAX}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 'clamp(2rem,4vw,3.5rem)', flexWrap: 'wrap', gap: '1rem' }}>
            <FadeUp>
              <p style={{ fontSize: '0.6rem', textTransform: 'uppercase', letterSpacing: '0.15em', color: INK_TERT, marginBottom: 8, fontWeight: 600 }}>Strategies in Action</p>
            ...
            <FadeUp delay={0.1}>
              <button onClick={() => go(setPage, 'work')} style={{ fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.14em', fontWeight: 600, color: INK_SEC, background: 'none', border: 'none', cursor: 'pointer', padding: 0, transition: 'color 0.2s' }}
                onMouseEnter={e => e.currentTarget.style.color = BLUE}
                onMouseLeave={e => e.currentTarget.style.color = INK_SEC}>All Work →</button>
            </FadeUp>
          </div>
          <div className="card-grid">{caseStudies.map((s, i) => <CaseStudyCard key={s.id} study={s} index={i} />)}</div>
        </div>
      </section>
        ...
        <div style={MAX}>
          <div style={{ marginBottom: 'clamp(2rem,4vw,3.5rem)' }}>
            <FadeUp>
              ...
              <p style={{ fontSize: '0.95rem', color: INK_BODY, maxWidth: 520, lineHeight: 1.7, marginTop: '0.75rem' }}>A glimpse in to the process of taking projects from "opportunity" to release. The "why, how, and what".</p>
            </FadeUp>
          </div>
          <div className="designs-grid">{designs.map((d, i) => <DesignCard key={d.id} design={d} index={i} />)}</div>
        </div>
      </section>
